defmodule RowWeights do
  ...
  def row_weights(arr) do
    {Enum.sum(evens(arr)), Enum.odds(evens(arr))}
  end

  def odds(arr), do: arr |> Stream.drop_every(2)
  def evens(arr), do: arr |> Stream.take_every(2)
end
...
defmodule RowWeights do
  ...
  """
  def row_weights(arr) do
    {Enum.sum(짝수들(arr)), Enum.sum(홀수들(arr))}
  end
